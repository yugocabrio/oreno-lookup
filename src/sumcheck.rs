...
impl<'a> Sumcheck<'a> {
    ...
    pub fn execute(&self) -> Result<(), &'static str> {
        ...
        for i in 0..self.num_variables {
            ...
            let s_0 = univariate_poly.poly_evaluate(&x0)?;
            let s_1 = univariate_poly.poly_evaluate(&x1)?;
            println!(
                "一変数多項式の評価: S_0 = {:?}, S_1 = {:?}",
                s_0, s_1
            );
            ...
            claimed_sum = univariate_poly.poly_evaluate(r_i)?;
    ...
    fn compute_univariate_polynomial(
        ...
        let prime = &self.polynomial.terms.values().next().unwrap().prime;
        let mut poly = Polynomial::new(vec![FieldElement::zero(prime)]);
        ...
        for bits in 0..total_assignments {
            ...
            // 各項を計算
            let mut term_poly = Polynomial::new(vec![FieldElement::zero(prime)]);
            ...
            for (exponents, coeff) in &self.polynomial.terms {
                ...
                let mono_poly = Polynomial::new(term_poly_coeffs);

                term_poly = term_poly.poly_add(&mono_poly)?;
            }

            poly = poly.poly_add(&term_poly)?;
        }
